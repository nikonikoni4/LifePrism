...
```python
{
    ...
    'class_by_default': TEXT,             # 默认分类（工作/学习/其他）
    'class_by_goals': TEXT,               # 根据目标分类
...
```python
{
    ...
    'title': TEXT,                        # 窗口标题
    'class_by_default': TEXT,             # 默认分类
    'class_by_goals': TEXT,               # 目标分类
...
- **现状**: 分类信息直接以文本形式存储在 `class_by_default` 和 `class_by_goals` 字段中
- **影响**: 无法支持前端的分类CRUD操作
...
#### 1. 数据迁移
- 需要将现有的 `class_by_default` 和 `class_by_goals` 文本数据迁移到新的分类表
...
3. ✅ 迁移现有的 `class_by_default` 数据到新的分类ID
...
2. **数据迁移**（如有需要）
   - 将旧的 `class_by_default` 数据映射到新的分类 ID
   - 编写迁移脚本
