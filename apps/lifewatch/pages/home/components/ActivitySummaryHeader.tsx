...
const getActivitySummary = async (
    ...
    try {
        const response = await ActivityAPI.getActivitySummary(
            centerDate, historyNumber, futureNumber,
            categoryId || undefined,
            subCategoryId || undefined
        );
        console.log('ActivitySummaryData:', response);
        const dailyActivities = response.activity_summary?.dailyActivities || [];
        const totalDay = historyNumber + futureNumber + 1;
        // 计算当天总活跃时间
        const todayData = dailyActivities.find(d => d.date === centerDate);
        const todayMinutes = todayData ? Math.floor(todayData.duration / 60) : 0;
        const hours = Math.floor(todayMinutes / 60);
        const mins = todayMinutes % 60;
        const TodayTotalActiveTime = `${hours}h ${mins}m`;
...
// Helper function to process ActivitySummaryData data (when passed as prop)
...
interface ActivitySummaryHeaderProps {
    ...
    activitySummaryData?: ActivitySummaryData; // Optional: if provided, use this data instead of fetching
    onRefresh?: () => void;
}
...
const ActivitySummaryHeader: React.FC<ActivitySummaryHeaderProps> = ({ selectedDate, onDateChange, activitySummaryData, onRefresh }) => {
    ...
    React.useEffect(() => {
        ...
        if (activitySummaryData && !hasFilter) {
            ...
            // 计算当天总活跃时间
            const todayData = activitySummaryData.dailyActivities?.find(d => d.date === selectedDate);
            const todayMinutes = todayData ? Math.floor(todayData.duration / 60) : 0;
            const hours = Math.floor(todayMinutes / 60);
            const mins = todayMinutes % 60;
            setTodayTotalActiveTime(`${hours}h ${mins}m`);
            setIsLoading(false);
            return;
        }

        // Otherwise, fetch data from API (with or without filters)
    ...
    return (
        <div className="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-gray-100 mb-8 animate-fade-in w-full">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <CategoryFilter
                        value={filter}
                        onChange={setFilter}
                        buttonClassName={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border rounded-xl text-sm font-semibold hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm ${filter.categoryId
            ...
            <div className={`relative pt-6 border-t border-dashed border-gray-100 transition-opacity duration-300 ${isLoading ? 'opacity-50' : 'opacity-100'}`}>
