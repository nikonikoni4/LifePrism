...
import { TopAppData, TopTitleData } from '../types';
...
interface ActivityDetailsWidgetProps {
    selectedDate: string;
    topApps?: TopAppData[];  // Optional: if provided, use this data instead of fetching
    topTitles?: TopTitleData[];  // Optional: if provided, use this data instead of fetching
}

const ActivityDetailsWidget: React.FC<ActivityDetailsWidgetProps> = ({ selectedDate, topApps: propsTopApps, topTitles: propsTopTitles }) => {
    const [topApps, setTopApps] = useState<TopAppData[]>([]);
    const [topWindows, setTopWindows] = useState<TopTitleData[]>([]);
