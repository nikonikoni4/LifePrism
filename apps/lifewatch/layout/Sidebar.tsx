...
const Sidebar: React.FC<SidebarProps> = ({ onChatToggle, isCollapsed = false, onToggle }) => {
  const location = useLocation();
  const navigate = useNavigate();

  // Determine current active page based on path
  const currentPage = useMemo(() => {
    const path = location.pathname;
    if (path === '/' || path === '/home') return 'home';
    return path.substring(1); // e.g. 'timeline'
  }, [location.pathname]);

  const navItems = [
    { id: 'home', icon: Home, label: 'Home', path: '/' },
    { id: 'timeline', icon: Clock, label: 'Timeline', path: '/timeline' },
    { id: 'category', icon: Tag, label: 'Category', path: '/category' },
    { id: 'reports', icon: FileBarChart, label: 'Reports', path: '/reports' },
    { id: 'usage', icon: DollarSign, label: 'Usage', path: '/usage' },
    // Removed Goals, Habits, Settings as they are separate modules now
    // { id: 'test', icon: FlaskConical, label: 'Test Lab', path: '/test' },
  ];
