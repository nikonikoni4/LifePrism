...
export const HabitFormDialog: React.FC<HabitFormDialogProps> = ({
    ...
    const onSubmit = async (data: HabitFormData) => {
        const payload = {
            ...
            frequency: {
                ...
                specificDays: data.frequencyType === 'custom' ? data.specificDays : undefined
            }
        };
    ...
    // calculate preview
    const calculatePreview = (level: number, type: FrequencyType, days: number[]) => {
            ...
            {isOpen && (
                ...
                >
                    ...
                    >
                        ...
                        {/* Form Body */}
                        <form id="habit-form" onSubmit={handleSubmit(onSubmit)} className="overflow-y-auto px-6 py-5 flex-1 space-y-5">
                            {/* Warning if editing frequency/level */}
                            {isEditMode && (habit.currentLevel !== initialLevel || habit.frequency.type !== frequencyType || (habit.frequency.specificDays || []).join(',') !== (specificDays || []).join(',')) && (
                            ...
                            <div className="space-y-1.5">
                                <label className="text-sm font-semibold text-slate-700 block">习惯名称</label>
                                <input
                                ...
                                <div className="flex bg-slate-100 p-1 rounded-xl">
                                    {FREQUENCY_OPTIONS.map(opt => (
                                        <button
                                            ...
                                            className={`flex-1 py-1.5 px-2 rounded-lg text-sm font-medium transition-all ${frequencyType === opt.value
                                    ...
                                    {frequencyType === 'custom' && (
                                        ...
                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                const newDays = isSelected
                                                                    ...
                                                                    : [...specificDays, day.value].sort((a, b) => a - b);
                                                                setValue('specificDays', newDays);
                                                            }}
                                                            className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium transition-all shadow-sm ${isSelected
                                            ...
                                            {frequencyType === 'custom' && specificDays.length === 0 && (
                                                <p className="text-xs text-red-500 mt-2">请至少选择一天</p>
                                            )}
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 flex items-center justify-between">
                                    <span className="text-sm text-slate-600 font-medium">设定起始等级</span>
                                    <div className="flex gap-1.5">
                                        {[0, 1, 2, 3, 4].map(level => (
                                            <button
                                                ...
                                                className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold transition-all ${initialLevel === level
                        ...
                        <div className="px-6 py-4 bg-slate-50/80 border-t border-slate-100 shrink-0">
                            ...
                            <div className="mb-4 flex items-center justify-between px-3 py-2 bg-indigo-50/50 rounded-lg border border-indigo-100/50">
                                <span className="text-xs font-medium text-indigo-800">挑战预览</span>
                            ...
                            <div className="flex justify-end gap-3">
                                ...
                                <button
                                    type="submit"
                                    form="habit-form"
                                    disabled={frequencyType === 'custom' && specificDays.length === 0}
