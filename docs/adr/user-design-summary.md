...
last_updated: 新增启动慢优化分阶段策略的设计原则（风险递进驱动方案分阶段执行、暂缓方案以 ADR 形式记录关键约束）
...
- **风险递进驱动方案分阶段执行**：当多个修复方案"风险-收益"不对称（低风险方案可能已足够解决问题、高风险方案收益最大但副作用多）时，先执行低风险方案观察实际效果，再决定是否承担高风险方案的副作用。判断标准：① 高风险方案是否需要破坏现有 ADR 契约或重设锁/资源生命周期；② 低风险方案修复后是否能直接判定高风险方案是否必要。暂缓方案以 ADR 形式记录关键约束（锁内移、task 引用、shutdown 处理等），让未来重启方案时有明确执行清单。驱动 [2026-08-19-startup-optimization-phased-strategy.md](./2026-08-19-startup-optimization-phased-strategy.md) 的"1+3 先做、2 暂不修改只记录"分阶段策略
...
- **多方案优化按风险递进执行，暂缓方案必须 ADR 记录约束清单**——当同时面对多个修复方案且风险不对称时，先执行低风险方案观察效果；暂缓的高风险方案必须以 ADR 形式记录"重新启动时必须满足的约束清单"（锁内移、task 引用持有、shutdown 处理、SSH 隧道时序、ADR 同步更新等），让未来重启方案时不需要重新调研。判断标准：暂缓方案的副作用是否会因时间推移而被遗忘导致重启时遗漏关键约束
