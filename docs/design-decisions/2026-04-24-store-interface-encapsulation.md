...
last_updated: 明确 storage store 接口强封装决策，禁止 service 层通过 .provider 访问底层实现
abstract: 记录 storage 层在 provider/aggregator 并存阶段的接口边界决策，统一上层只通过 store 方法访问，降低混用与误用风险。
status: stable
---
...
| 版本 | 更新内容 |
| ---- | -------- |
| 1.0 | 创建 store 接口强封装决策文档 |

## 背景介绍/现状

当前项目的 `storage` 层目标是“对外统一 `store` 接口”，上层通过 `from lifeprism.storage import xxx_store` 使用数据能力。  
...
1. 认知成本高：调用方需要先判断当前 store 是否可直接调用，还是需要穿透 `.provider`。
2. 规范不一致：不同模块（如单表 store 与聚合 store）暴露能力形态不同，容易形成“局部经验迁移错误”。
3. AI/协作误用风险高：在自动补全或 AI 生成代码时，常出现一处走 store、一处走 `.provider` 的混用。
...
2. 对于上层需要且高频的能力，由 aggregator/store 增加同名或语义清晰的方法进行透传。
3. 透传范围采用白名单策略（优先 CRUD 与常用查询），不做无边界全量暴露。
4. 当存在特殊底层能力需求时，应先在 store 层新增正式接口，而不是由业务层绕过边界直连 provider。

## 涉及范围

- `lifeprism/storage/__init__.py` 中对外导出的 `*_store` 访问约定
- 聚合类（如 `CategoryAggregator`）的对外方法设计
- server service 层对 storage 的调用方式
- 团队协作、代码评审与 AI 生成代码的接口一致性约束

## 为什么要做这个决定

1. 降低认知负担：统一“只看 store 接口”，减少调用前判断分支。
...
- 项目内架构说明：`lifeprism/storage/__init__.py`（Store 统一对外接口注释）
