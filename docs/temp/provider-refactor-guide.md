...
你需要为下面的任务创建一个todolist

```
# 构建快照测试
1. 找到需要创建/重构 的 provider对应表 在service的使用位置
2. 确保当前数据库该表是有数据的（避免快照测试的数据为空）
3. 为当前的service构建快照测试，并运行

# 重构provider
4. 按照下面具体章节要求构造provider
5. 为provider的每个方案构建单元测试
6. 进行单元测试，确保所构建方便本身无错误

# 应用新provider，替换旧的使用
7. 依据步骤1提提供的信息，替换对应service脚本中provider的使用
8. 运行快照测试
9. 快照测试成功之后提交代码
