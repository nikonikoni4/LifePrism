...
last_updated: 初始版本
abstract: 计划书 MD 文档与数据库双向同步规则，定义 todoblock 格式、锚点规范、缩进父子关系算法、同步触发时机、三数据源架构（编辑器-MD-DB）和前端保存Hook机制
---

# 计划书 MD 文档同步规则

计划书（PlanDoc）使用 Markdown 文件存储任务列表，通过 `plandoc_sync_service.py` 实现 MD 与数据库的双向同步，`taskpool_service.py`（Todo CRUD）负责调用同步服务进行 MD 回写。
...
| 字段 | 说明 |
|------|------|
| `id` | 任务 ID（TEXT，格式：`t-{uuid[:8]}`，与 MD 锚点统一） |
| `plan_doc_id` | 关联的计划书 ID |
| `parent_id` | 父任务 ID（TEXT） |
...
- **MD ↔ DB**：后端通过 `plandoc_sync_service` 实现双向同步（`sync_plan_doc`），`taskpool_service`（Todo CRUD）调用回写函数（`update_todo_with_writeback`）
...
用户点击同步按钮
  ...
  → todoApi.syncPlanDoc(dry_run)    ← 预检
  → [用户确认删除/保留]
  → todoApi.syncPlanDoc(执行)       ← MD → DB 同步
  → loadTasks()                      ← 刷新任务池列表
...
- `frontend/apps/goals/apis/todoApi.ts` - Todo API（fetchTaskPool/syncPlanDoc/updateTodo 等）
...
| 模块 | 职责 |
|------|------|
| `plandoc_sync_service.py` | MD 文件 I/O、正则解析、todoblock 处理、sync_plan_doc、状态回写、系统展示区 |
| `taskpool_service.py` | Todo CRUD、数据转换、MD 回写调度（调用 plandoc_sync_service） |

调用关系：
- `taskpool_api.py` → `plandoc_sync_service.sync_plan_doc()`、`plandoc_sync_service.regenerate_summary()`
- `todos_api.py` → `taskpool_service.*`（所有 Todo CRUD 操作）
- `taskpool_service` → `plandoc_sync_service.insert_todo_to_md()`、`update_todo_in_md()`、`delete_todo_from_md()`、`writeback_completion_to_md()`、`writeback_uncomplete_to_md()`

## 待清理问题（最终阶段）

以下是代码审查中发现的存量问题，非本次拆分引入，留待最终清理阶段处理：

1. **`taskpool_service.get_taskpool` 的 `state` 参数默认值违规**：Service 函数 `state: Optional[str] = "all"` 违反 CLAUDE.md 规则（Service 函数禁止写参数默认值）。应改为 `state: Optional[str] = None`，默认值由 API 层 `Query(default="all")` 控制。

2. **`validate_todo_integrity` 和 `safe_update_todo_to_md` 无调用者**：这两个函数在 `plandoc_sync_service.py` 中定义但无任何调用点。按"0 个引用 → 直接删除"原则应清理，或标注为预留接口。
