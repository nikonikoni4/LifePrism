...
last_updated:
abstract: Windows 本地 ↔ Linux 云端双向数据同步模块规格，定义数据库同步、文件同步、心跳管理与消息路由、云端配置初始化、认证安全的技术契约
status: draft
module: sync
---
...
- **文件双向同步**：session、channel/wechat/account.json 等 11 个目录/文件的同步
...
- 文件同步（gzip 压缩 + base64 编码）
- 心跳状态管理（纯内存，15 分钟超时）
- 消息路由（本地在线时云端跳过消息处理）
- API 端点：pull / push / heartbeat / pull-files / push-files
...
- [ ] 动态表（custom_{slug}）自动发现并参与同步
- [ ] 所有步骤成功后更新 `last_sync_time`

### 文件同步

- [ ] 文件同步在数据库同步之后执行
- [ ] 增量检测：mtime > last_sync_time 的文件才同步
- [ ] gzip 压缩 + base64 编码传输
- [ ] LWW 冲突解决：本地 mtime 更新时跳过写入
- [ ] `channel/wechat/account.json` 作为单文件参与同步
...
- lifeprism/repository/sync_repository.py
  ...
  - sync_repository.SyncRepository.get_custom_record_slugs:1045
  - sync_repository.SyncRepository.get_primary_key_field:958
  - sync_repository.SyncRepository.get_unique_fields:985
  - sync_repository.SyncRepository.has_updated_at:1028
  - sync_repository.SyncRepository.count_rows:158
  - sync_repository.SyncRepository.count_rows_batch:195
</key_function>
...
| 接口 | 说明 | 约束 |
|------|------|------|
| `sync_once(tables, directories)` | 执行一次完整同步 | Pull → Push（数据库+文件），全部成功才更新 last_sync_time |
...
- lifeprism/server/api/sync_cloud_api.py
  ...
  - sync_cloud_api.sync_pull_files:308
  - sync_cloud_api.sync_push_files:388
...
| 动态表 | custom_{slug}（运行时从 custom_record_types 发现） |
...
agent/                            # Agent 配置
assets/                           # 资源文件
channel/wechat/account.json      # 微信 session_id（必须同步）
diary/                            # 日记 MD 文件
docs/                             # 用户文档
external_files/                   # 外部文件
plan/                             # 计划文件
...
- **时区一致性要求**：本地和云端必须使用相同时区生成 `updated_at` 和 `mtime`，否则 LWW 比较会失效。当前假设服务器和本地都使用本地时区，但如果云端部署在海外服务器，可能需要全局时区配置
- 消息路由在网络分区场景下可能产生重复回复（概率 < 1%）
- 文件同步不做行级 diff（JSONL 文件），每次传输完整文件
...
- **时区配置缺失**：当前没有全局时区配置项，依赖系统默认时区。如果云端服务器在海外，可能导致时间戳比较错误
