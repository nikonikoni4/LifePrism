...
- 触发规则：当任务涉及 `lifeprism/repository/providers` 基础 CRUD 命名统一与调用修复时读取
- 内容摘要：统一 provider 基础 CRUD 方法命名到 create-table-rules 规范，并同步修复跨层调用与测试。

### usage-service-tokens-repository-migration

- updated_at: 2026-04-24
- path: `docs/plans/2026-04-24-usage-service-tokens-repository-migration.md`
- 触发规则：当任务涉及 usage_service 从 server_lw_data_provider 迁移到 tokens_usage_store 时读取
- 内容摘要：将 token 使用统计切换到 repository 层通用查询并在 service 层完成按日/按 mode 聚合适配，保持返回结构不变。
