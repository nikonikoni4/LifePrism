...
const getActivitySummary = async (
  ...
  try {
    const response = await ActivityAPIV2.getActivitySummary(
      centerDate, historyNumber, futureNumber,
      categoryId || undefined,
      subCategoryId || undefined
    );
    console.log('ActivitySummaryData V2:', response);
...
// Helper function to process ActivitySummaryDataV2 data (when passed as prop)
const processActivitySummaryData = (
  activitySummaryData: ActivitySummaryDataV2,
  centerDate: string,
  historyNumber: number,
  futureNumber: number
) => {
...
interface ActivitySummaryHeaderProps {
  ...
  activitySummaryData?: ActivitySummaryDataV2; // Optional: if provided, use this data instead of fetching
  onRefresh?: () => void;
}
...
const ActivitySummaryHeader: React.FC<ActivitySummaryHeaderProps> = ({ selectedDate, onDateChange, activitySummaryData, onRefresh }) => {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        const cats = await CategoryAPIV2.getAllCategories();
        setCategories(cats);
      } catch (error) {
  ...
  const handleTimeRangeSync = async () => {
    ...
    try {
      ...
      await SyncAPIV2.syncByTimeRange({
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button
            onClick={async () => {
              ...
              setIsSyncing(true);
              try {
                await SyncAPIV2.incrementalSync();
                if (onRefresh) {
                  onRefresh();
                }
              } catch (error) {
