...
const GoalsWidget: React.FC<{ selectedDate: string }> = ({ selectedDate }) => {
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 z-10 pr-2">
        {goals.map((goal) => (
          <div
            ...
            className={`group flex items-center p-3.5 rounded-2xl border transition-all duration-200 cursor-pointer ${goal.completed
              ? 'bg-gray-50 border-transparent opacity-60'
              : 'bg-white border-gray-100 hover:border-blue-200 hover:shadow-md hover:shadow-blue-500/5'
              }`}
          >
            <div className={`w-6 h-6 rounded-lg border-2 mr-4 flex-shrink-0 flex items-center justify-center transition-all duration-300 ${goal.completed
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-0.5">
                <p className={`text-sm font-semibold truncate transition-all ${goal.completed ? 'text-gray-400 line-through decoration-2' : 'text-slate-700'
                  }`}>
                  {goal.text}
                </p>
