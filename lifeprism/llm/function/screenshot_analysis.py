...
from lifeprism.llm.exceptions import LLMOutputParseError, LLMResponseError
...
def get_active_screenshots(seg_start: str, seg_end: str) -> list[dict[str, Any]]:
    ...
    return screen_capture_repository.query_screenshots(
        start_time=seg_start, end_time=seg_end, capture_reason="active"
    )
    ...
    try:
        ...
        if is_multi:
            ...
            result, _ = map_cache_repository.query_multi_purpose_map_cache(
                QueryOptions(
                    filters={"app": app, "title": title}, fields=["category_id", "app_description"]
                )
            )
            if result:
        ...
        if is_ignored and category_id:
            ...
            category_name = category_info.get("name", "未分类") if category_info else "未分类"
...
async def analyze_chunk_screenshots(
    chunk: dict[str, str], screenshots: list[dict[str, Any]], todolist: str | None = None
) -> dict[str, Any] | None:
    """分析单个 chunk 的截图语义
    ...
    if len(screenshots) > MAX_SCREENSHOTS_PER_CHUNK:
        ...
        logger.warning(
            "截图数量超过 %s，已截断至 %s 张", MAX_SCREENSHOTS_PER_CHUNK, len(screenshots)
        )
...
async def screenshot_analysis(
    ...
    logger.info(
        "开始截图语义分析: %s -> %s, 频率等级=%s, chunk大小=%s分钟",
        start_time,
        end_time,
        frequency_level,
        chunk_minutes,
    ...
    for i, chunk in enumerate(all_chunks, 1):
        ...
        analysis_tasks.append(analyze_chunk_screenshots(chunk, screenshots, todolist))
...
async def _behavior_summary(
    start_time: str, end_time: str, screen_count: int, behavior: str, todolist: str
) -> dict[str, Any]:
    """
    对输入的行为内容进行总结
    args ：
    ...
    try:
        ...
        logger.error(
            ...
            start_time,
            end_time,
            response_content[:500] if "response_content" in dir() else "(unknown)",
        )
        raise LLMOutputParseError(
            expected_fields=["behavior_summary", "title"],
            actual_keys=[],
...
async def screenshot_behavior_summary(
    analysis_results_list: list[dict[str, Any]], todolist: str = ""
) -> list[dict[str, str]]:
    """
    对单个chuck分析进行合并总结
    args:
...
if __name__ == "__main__":
    ...
    async def test_single_chunk():
        ...
        for sc in screenshots:
            print(
                f"  - {sc['captured_at']} | {sc['window_app']} | {sc.get('window_title', '')[:50]}"
            )
