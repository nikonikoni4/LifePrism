...
def write_behavior_md(
    ...
    # 匹配精确的日期标题
    pattern = re.compile(rf'^(##\s+{re.escape(date)}\s*)$', re.MULTILINE)
    ...
    # 日期标题存在，找到紧接着的下一个日期匹配
    next_pattern = re.compile(r'^##\s+\d{4}-\d{2}-\d{2}\s*$', re.MULTILINE)
    next_match = next_pattern.search(full_content, match.end())
    
    next_pos = next_match.start() if next_match else len(full_content)
    
    # 提取当前日期的已有内容块
    existing_block = full_content[match.end():next_pos]
    ...
    # 替换其中的内容
    new_full_content = full_content[:match.end()] + new_block + full_content[next_pos:]
...
def extract_behavior_md(
    ...
    # 因为第一个元素 (parts[0]) 是第一个二级标题前的所有内容（如文件大标题），我们不需要它
    # 后面的元素总是：索引为奇数的是"日期字符串"，紧随其后的偶数索引是"正文区块"
    for i in range(1, len(parts), 2):
        current_date_str = parts[i]
        ...
        if start_dt <= current_dt <= end_dt:
            ...
            if current_date_str in results:
                ...
                results[current_date_str] = content_block
...
def extract_behavior_logs_from_file(
    ...
    return extract_behavior_md(content, start_date, end_date)
