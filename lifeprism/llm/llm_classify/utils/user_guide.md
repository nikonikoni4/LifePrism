  ...
  "content": [
      {
        ...
        "items": ["颜色选择", "修改名称", "禁用类别", "删除类别", "添加类别","启用类别"],
        "content": [
            {
                "title": "颜色选择",
            ...
            {
                "title": "禁用类别按钮",
                "abstract": "禁用分类后，AI 将不再使用该分类进行新数据分类，但历史数据保留，仍可正常显示。注意：1. 禁用该类别之后，map cache里的缓存会被同步禁用。2. 若goal也与该类别关联，那么goal不会被纳入分类流程中。"
            },
            {
                "title": "启用类别按钮",
                "abstract": "启用分类后，AI 将恢复使用该分类进行自动归类。同时，系统会自动恢复该分类下之前被禁用的缓存记录（Map Cache）。\n\n**冲突处理机制**：如果在分类禁用期间，原本属于该分类的应用被归类到了其他类别（并产生了新的缓存），启用操作将会自动删除这些在禁用期间产生的较新缓存，优先恢复该应用原有的分类设置。\n\n**示例时间轴**：\n1. **T1 (正常)**: App A 被归类为「工作」（缓存有效）\n2. **T2 (操作)**: 用户禁用「工作」分类（T1缓存失效）\n3. **T3 (运行)**: App A 再次运行，因「工作」禁用，AI 将其归类为「其他」（产生新缓存）\n4. **T4 (恢复)**: 用户重新启用「工作」分类\n   👉 **结果**: 系统删除 T3 产生的「其他」缓存，恢复 T1 的「工作」缓存。"
            },
            {
                "title": "删除类别按钮",
  ...
  "content": [
      ...
      },
      {
        ...
        "content": [
            ...
            {
                "title": "跨日按钮",
                "abstract": "开启跨日功能后，该任务在完成之前会在每天的待办列表中持续显示；完成后只会显示在创建当天的记录中。适用于长期任务。当手动设置了预期结束时间时，也会自动开启跨日功能。"
            },
            {
                "title": "今日重点",
      ...
      },
      {
        "id": "plan",
        "title": "Plan（计划）",
        "abstract": "规划周期性工作安排，包含月计划和周计划两个层级。周计划界面包含任务池和周计划列表。",
        ...
        "content": [
            ...
            {
                "title": "任务池",
                "abstract": "任务池可以一次性创建多个待办任务，可以通过拖拽到具体的周计划中"
            }
        ]
      },
      {
        ...
        "content": [
            ...
            },
            {
                ...
                "content": [
                    ...
                    {
                        "title": "AI分类优先机制",
                        "abstract": "当目标绑定了某个分类后，AI 在分类新数据时会优先判断该数据是否与此目标相关。如果数据内容（应用名、窗口标题等）符合该目标的上下文，AI 会直接使用目标绑定的分类，跳过常规分类判断流程。这确保与特定目标相关的活动能够准确归类。注意：当目标绑定的分类被禁用后，该目标也不会再被纳入分类流程中。"
                    },
                    {
                        "title": "最佳实践",
            ...
            },
            {
                "id": "faq-goal-track",
                "title": "goal的时间追踪",
                "abstract":"1. 自动统计：goal的时间追踪依赖于分类绑定，在分类时会优先匹配goal。所有被goal匹配的事件数据都会被统计到goal的时间中。2. 手动设置：可以在map cache界面手动为单用途软件设置link_to_goal,该软件的使用时间会被统计到该goal中。若一个软件与多个goal相关，仅支持统一时间段内仅支持一个goal，通过在同步数据中选择不同的日期即可将不同的软件绑定到不同的goal来实现"
            }
        ]
      },
      {
...
```json
{
  ...
  "content": [
      ...
      {
        "id": "faq-log-link-goal",
        "title": "Q5: 新创建了一个goal，但是我想追踪的创建之前的数据，使得之前的数据可以被添加到时间统计中，我该怎么做？",
        "abstract": "1. 确保创建的goal已经设置了分类 2. 在category->map cache中找到该目标使用的软件 3. 点击编辑按钮，设置link_to_goal 4. 点击确认后，重新同步数据，即可将之前的数据添加到时间统计中",
        "keywords": ["link_to_goal", "log", "goal"]
      },
      {
        "id": "faq-link-multi-app-to-goal",
        "title": "Q6: 一个软件只能绑定一个目标吗？如果我创建了多个目标，一个软件可以绑定多个目标吗？",
        "abstract":"1. 对于单用途软件，一个软件只能绑定一个目标，对于多用途，一个软件可以绑定多个目标。但是目前分类只支持浏览器，不建议将其他软件设置为多用途。 2. 可以做到的是一个软件在不同时间段内绑定不同的目标，操作方法是：在category->map cache中找到该软件，点击编辑按钮，设置link_to_goal，然后点击确认，在同步数据中选择不同的日期即可将不同的软件绑定到不同的目标。举例：若我使用vs code在1号到5号之间是实现不要a，6号到10号之间是实现b，那么我可以在1号到5号之间将vs code绑定到a，6号到10号之间将vs code绑定到b，这样在统计1号到10号之间的时间时，vs code的时间会被分别统计到a和b中。"
      }
  ]
}
```
...
```json
{
  "id": "data-cleaning",
  "title": "数据清洗流程",
  "abstract": "系统从 ActivityWatch 获取原始数据后，经过清洗、标准化、缓存匹配等处理，最终输出可用于分类的事件数据。这是 AI 分类的前置流程。",
  "keywords": ["数据清洗", "数据标准化", "缓存匹配", "Map Cache", "单用途", "多用途"],
  "items": ["流程概览", "缓存匹配策略", "数据映射规则"],
  "content": [
      {
        "id": "cleaning-overview",
        "title": "流程概览",
        "abstract": "数据清洗流程：原始事件 → 事件转换（过滤+标准化） → 缓存匹配 → 待分类收集 → 输出。已匹配到缓存的事件直接获取分类，未匹配的进入待分类列表。"
      },
      {
        "id": "cache-matching",
        "title": "缓存匹配策略",
        "abstract": "根据应用类型采用不同匹配策略：单用途应用按应用名匹配，多用途应用按应用名+窗口标题组合匹配。匹配成功直接获取主分类、子分类和关联目标；匹配失败进入待分类流程。",
        "items": ["单用途应用", "多用途应用"],
        "content": [
            {
                "title": "单用途应用",
                "abstract": "只按应用名匹配。同一应用始终使用相同分类，如 VS Code 始终归类为开发工具。"
            },
            {
                "title": "多用途应用",
                "abstract": "按应用名+窗口标题组合匹配。同一浏览器打开不同网站会有不同分类，如 Chrome + github.com → 工作，Chrome + bilibili.com → 娱乐。"
            }
        ]
      },
      {
        "id": "data-mapping-rules",
        "title": "数据映射规则",
        "abstract": "核心映射规则：1) 标准化保证一致性，确保相同应用能匹配到同一缓存记录；2) 缓存优先，只有未命中的数据才调用 AI；3) AI 分类结果会写入 Map Cache，后续相同应用直接使用缓存。"
      }
  ]
}
```
