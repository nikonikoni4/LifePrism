...
"""
    ...
    async def _execute_tool_call(self, tool_call: dict) -> tuple[bool, str | None]:
        """
        异步执行单个工具调用
    ...
    async def _execute_node(self, node: NodeDefinition):
        """
        异步执行单个节点，包含工具调用循环
        ...
        # 过滤已用完额度的工具
        available_tools = [t for t in (node.tools or []) if self._can_use_tool(t)] or None
        
        # 创建 LLM（带可用工具）
        llm = self._create_llm_with_tools(available_tools)
        result = None
        ...
        while True:
            ...
            for tool_call in result.tool_calls:
                success, _ = await self._execute_tool_call(tool_call)
                if success:
                    executed_count += 1
    ...
    async def execute(self):
        ...
        for node in self.plan.nodes:
            content = await self._execute_node(node)
...
if __name__ == "__main__":
    ...
    async def main():
        plan, tools_limit = get_daily_summary_plan(
            "2026-01-05",
            json_path=r"D:\desktop\软件开发\LifeWatch-AI\lifeprism\llm\llm_classify\tests\data_driving_agent\pattern\daily_summary_plan.json",
            pattern_name="simple"
        )
        executor = AsyncExecutor(plan, "总结 2026-01-05 的使用情况", tools_limit=tools_limit)
