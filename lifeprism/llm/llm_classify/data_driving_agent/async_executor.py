# 异步执行器定义 V2
# 支持多线程 Context 和 3 种节点类型分发
# 数据输入输出完全由 data_in_thread 和 data_out_thread 控制
import asyncio
from lifeprism.llm.llm_classify.schemas.data_driving_schemas import (
...
class AsyncExecutor:
    ...
    支持特性:
    - 多线程 Context 消息隔离
    - 3 种节点类型分发执行 (llm-first, tool-first, planning)
...
"""
    ...
    async def _execute_tool_call_for_thread(self, tool_call: dict, thread_id: str) -> tuple[bool, str | None]:
        ...
        # 异步执行工具（在线程池中运行同步的工具调用）
        tool_result = await asyncio.to_thread(
            self.tools_map[tool_name].invoke, tool_args
        )
...
{node.task_prompt}"""
    ...
    async def _execute_tool_first_node(self, node: NodeDefinition) -> str:
        ...
        tool_result = await asyncio.to_thread(
            self.tools_map[node.initial_tool_name].invoke, tool_args
        )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        plan, tools_limit = load_plan_from_template(
            json_path=r"D:\desktop\软件开发\LifeWatch-AI\lifeprism\llm\llm_classify\tests\data_driving_agent_v2\patterns\daily_summary_plan.json",
            pattern_name="complex",
            date=date
        )
        executor = AsyncExecutor(plan, f"请帮我总结 {date} 的使用情况", tools_limit=tools_limit)
        result = await executor.execute()
        
        # 格式化输出
        logger.debug("\n" + "=" * 80)
        logger.debug("📊 AI 生成的行为总结")
        logger.debug("=" * 80 + "\n")
        logger.debug(result["content"])
        logger.debug("\n" + "=" * 80)
        logger.debug(f"📈 统计信息：共产生 {sum(len(msgs) for msgs in result['messages'].values())} 条消息")
        tokens = result["tokens_usage"]
        logger.debug(f"🔢 Tokens 使用: 输入={tokens['input_tokens']}, 输出={tokens['output_tokens']}, 总计={tokens['total_tokens']}")
        logger.debug("=" * 80)
