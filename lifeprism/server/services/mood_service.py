...
from lifeprism.storage import mood_store
...
def get_mood_types() -> MoodTypeListResponse:
    """获取所有心情类型"""
    items = mood_store.get_mood_types()
...
def create_mood_type(request: CreateMoodTypeRequest) -> Optional[MoodTypeItem]:
    ...
    new_id = mood_store.create_mood_type(data)
    if not new_id:
        return None
    item = mood_store.get_mood_type_by_id(new_id)
    if not item:
        return None
...
def update_mood_type(mood_type_id: str, request: UpdateMoodTypeRequest) -> Optional[MoodTypeItem]:
    ...
    """
    existing = mood_store.get_mood_type_by_id(mood_type_id)
    if not existing:
        return None
    ...
    if update_data:
        mood_store.update_mood_type(mood_type_id, update_data)

    item = mood_store.get_mood_type_by_id(mood_type_id)
...
def delete_mood_type(mood_type_id: str) -> bool:
    ...
    count = mood_store.count_entries_by_type(mood_type_id)
    if count < 0:
        raise ValueError("查询关联记录失败")
    if count > 0:
        raise ValueError(f"该心情类型下有 {count} 条记录，无法删除")

    return mood_store.delete_mood_type(mood_type_id)
...
def get_mood_entries(start_date: Optional[str] = None, end_date: Optional[str] = None) -> MoodEntryListResponse:
    ...
    """
    items = mood_store.get_mood_entries(start_date, end_date)
...
def get_mood_entry(entry_id: str) -> Optional[MoodEntryItem]:
    ...
    """
    item = mood_store.get_mood_entry_by_id(entry_id)
    if not item:
        return None
...
def create_mood_entry(request: CreateMoodEntryRequest) -> Optional[MoodEntryItem]:
    ...
    """
    mood_type = mood_store.get_mood_type_by_id(request.mood_type_id)
    if not mood_type:
    ...
    new_id = mood_store.create_mood_entry(data)
    if not new_id:
        return None
    item = mood_store.get_mood_entry_by_id(new_id)
...
def update_mood_entry(entry_id: str, request: UpdateMoodEntryRequest) -> Optional[MoodEntryItem]:
    ...
    """
    existing = mood_store.get_mood_entry_by_id(entry_id)
    if not existing:
        return None
    ...
    if update_data:
        mood_store.update_mood_entry(entry_id, update_data)
...
def delete_mood_entry(entry_id: str) -> bool:
    ...
    Returns:
        bool: 是否成功
    """
    return mood_store.delete_mood_entry(entry_id)
...
def get_mood_impacts() -> MoodImpactListResponse:
    """获取所有影响因素"""
    items = mood_store.get_mood_impacts()
...
def delete_mood_impact(impact_id: int) -> bool:
    ...
    Returns:
        bool: 是否成功
    """
    return mood_store.delete_mood_impact(impact_id)
