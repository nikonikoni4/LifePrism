...
def create_mood_type(request: CreateMoodTypeRequest) -> Optional[MoodTypeItem]:
    ...
    new_id = mood_repository.create_mood_type(data)
    if not new_id:
        return None
    item = mood_repository.get_mood_type_by_id(new_id)
    if not item:
        return None
...
def update_mood_type(mood_type_id: str, request: UpdateMoodTypeRequest) -> Optional[MoodTypeItem]:
    ...
    """
    existing = mood_repository.get_mood_type_by_id(mood_type_id)
    if not existing:
        return None
    ...
    if update_data:
        mood_repository.update_mood_type(mood_type_id, update_data)

    item = mood_repository.get_mood_type_by_id(mood_type_id)
...
def delete_mood_type(mood_type_id: str) -> bool:
    ...
    count = mood_repository.count_entries_by_type(mood_type_id)
    if count < 0:
        raise ValueError("查询关联记录失败")
    if count > 0:
        raise ValueError(f"该心情类型下有 {count} 条记录，无法删除")

    return mood_repository.delete_mood_type(mood_type_id)
...
def get_mood_entry(entry_id: str) -> Optional[MoodEntryItem]:
    ...
    """
    item = mood_repository.get_mood_entry_by_id(entry_id)
    if not item:
        return None
...
def create_mood_entry(request: CreateMoodEntryRequest) -> Optional[MoodEntryItem]:
    ...
    if not new_id:
        return None
    item = mood_repository.get_mood_entry_by_id(new_id)
...
def update_mood_entry(entry_id: str, request: UpdateMoodEntryRequest) -> Optional[MoodEntryItem]:
    ...
    """
    existing = mood_repository.get_mood_entry_by_id(entry_id)
    if not existing:
        return None
...
def delete_mood_entry(entry_id: str) -> bool:
    ...
    Returns:
        bool: 是否成功
    """
    return mood_repository.delete_mood_entry(entry_id)
...
def create_mood_impact(request: CreateMoodImpactRequest) -> Optional[MoodImpactItem]:
    ...
    data = request.model_dump()
    new_id = mood_repository.create_mood_impact(data)
    if new_id is None:
        return None
    # 查询刚创建的记录
    items = mood_repository.get_mood_impacts()
...
def delete_mood_impact(impact_id: int) -> bool:
    ...
    Returns:
        bool: 是否成功
    """
    return mood_repository.delete_mood_impact(impact_id)
