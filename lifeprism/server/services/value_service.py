...
def get_values() -> ValueListResponse:
    ...
    return ValueListResponse(
        items=[_convert_to_value_item(item) for item in items]
    )


def get_value_detail(value_id: str) -> Optional[ValueDetailItem]:
    """
    获取价值详情（含关联承诺列表）
...
def create_value(request: CreateValueRequest) -> Optional[ValueItem]:
    """
    创建价值
...
def update_value(value_id: str, request: UpdateValueRequest) -> Optional[ValueItem]:
    """
    更新价值（部分更新）
