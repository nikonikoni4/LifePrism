...
def _get_behavior_content(date: str) -> str:
    ...
    """
    from lifeprism.llm.utils.md_os import extract_date_md, read_md
...
def get_daily_report(date: str, force_refresh: bool) -> DailyReportResponse:
    ...
    sunburst_data = _calc_sunburst_data(
        start_date=date, end_date=date, title="今日时间分布", total_range_minutes=1440
    )
...
def _calc_comparison_data(current_start: str, current_end: str, period_type: str = "daily"):
    ...
    # 转换为 Schema 格式
    category_items = [
        CategoryComparisonItem(**item) for item in raw_data.get("category_comparison", [])
    ]

    goal_items = [GoalComparisonItem(**item) for item in raw_data.get("goal_comparison", [])]
...
def _calc_sunburst_data(
    start_date: str, end_date: str, title: str, total_range_minutes: int
) -> TimeOverviewData | None:
    """
...
def _calc_todo_stats(start_date: str, end_date: str) -> TodoStatsData:
    ...
    try:
        ...
        for i in range(days):
            ...
            todos = todo_repository.get_todos_by_date(current_date, include_cross_day=False)
            ...
            procrastination += sum(
                ...
                and t.get("expected_finished_at")
                and t["expected_finished_at"] < current_date
            )
        ...
        return TodoStatsData(
            total=total, completed=completed, pending=pending, procrastination_rate=round(rate, 1)
        )
...
def _calc_goal_progress(start_date: str, end_date: str) -> list[GoalProgressData]:
    ...
    try:
        ...
        for goal in goals:
            ...
            for i in range(days):
                current_date = (start_dt + timedelta(days=i)).strftime("%Y-%m-%d")
                all_todos = todo_repository.get_todos_by_date(current_date, include_cross_day=True)
            ...
            todo_list = [
                GoalTodoItem(
                    id=t["id"], content=t["content"], completed=t.get("state") == "completed"
                )
                for t in goal_todos
            ]
...
def _build_category_level(
    df: pd.DataFrame, name_map: dict, is_main_category: bool, group_field: str = "category_id"
) -> TimeOverviewData:
    """构建分类级别数据"""
    ...
    for _, row in stats.iterrows():
        ...
        pie_data.append(
            ChartSegment(key=cat_id, name=name, value=minutes, color=item_color, title="")
        )

        bar_keys.append(BarConfig(key=cat_id, label=name, color=item_color))
...
def _build_app_level(
    ...
    for _i, (app_name, minutes) in enumerate(top_5.items()):
        ...
        pie_data.append(
            ChartSegment(
                key=app_name, name=app_name, value=int(minutes), color=app_color, title=top_titles
            )
        )

        bar_keys.append(BarConfig(key=app_name, label=app_name, color=app_color))

    if other_value > 0:
        other_color = "#9CA3AF"
        pie_data.append(
            ChartSegment(
                key="Other", name="Other Apps", value=int(other_value), color=other_color, title=""
            )
        )
        bar_keys.append(BarConfig(key="Other", label="Other", color=other_color))
...
def _build_empty_weekly_trend(start_date: str = None) -> list[dict[str, Any]]:
    ...
    if start_date:
        ...
        start_dt = datetime.strptime(start_date, "%Y-%m-%d").date()
        return [
            {"label": name, "date": str(start_dt + timedelta(days=i))}
...
def _build_empty_heatmap(start_date: str, end_date: str) -> list[HeatmapDataItem]:
    ...
    for i in range(days):
        ...
        result.append(
            HeatmapDataItem(
                date=current_date.strftime("%Y-%m-%d"), total_minutes=0, category_breakdown=None
            )
        )
    return result
...
def _weekly_dict_to_response(
    data: dict[str, Any], week_start_date: str, week_end_date: str
) -> WeeklyReportResponse:
    """将数据库记录转换为周报告响应模型"""
    sunburst_data = None
    if data.get("sunburst_data"):
...
def _monthly_dict_to_response(
    data: dict[str, Any], month_start_date: str, month_end_date: str
) -> MonthlyReportResponse:
    """将数据库记录转换为月报告响应模型"""
    sunburst_data = None
    if data.get("sunburst_data"):
    ...
    if weekly_dec_31 and monthly_dec_31:
        ...
        for cat in sorted(all_cats):
            ...
            if diff != 0:
                has_diff = True
            print(
                f"   {status} {cat}: 周={w_val}分钟, 月={m_val}分钟, 差异={diff}分钟 ({diff / 60:.2f}小时)"
            )
