...
def slice_events_by_time_range(
    df: pd.DataFrame, range_start: datetime, range_end: datetime
) -> pd.DataFrame:
    """
...
def _calculate_block_stats(
    ...
    if category_level == "main":
        ...
        def color_getter(cat_id):
            return get_timeline_category_color(cat_id, is_sub_category=False)
        ...
        def color_getter(cat_id):
            return get_timeline_category_color(cat_id, is_sub_category=True)
    ...
    if not block_df.empty:
        ...
        stats = block_df.groupby(group_field).agg({"duration_seconds": "sum"}).reset_index()
...
def _build_category_level_data(
    ...
    if include_idle and idle_minutes > 0:
        ...
        bar_keys.append({"key": "Idle", "label": "Idle", "color": idle_color})
...
def _build_empty_time_overview(
    title: str, sub_title: str, range_start: datetime = None, range_end: datetime = None
) -> TimeOverviewData:
    """构建空的时间概览响应（包含空闲时间）"""
    from datetime import timedelta
    ...
    if range_start and range_end:
        ...
        pie_data = [
            ChartSegment(key="idle", name="Idle", value=idle_minutes, color="#E5E7EB", title="")
        ]
        bar_keys = [BarConfig(key="Idle", label="Idle", color="#E5E7EB")]
    else:
        # 默认 24 小时显示
