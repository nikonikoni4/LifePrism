...
class BeingProvider(LWBaseDataProvider):
    ...
    def get_by_id(self, record_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error("获取记录失败: record_id=%s, error=%s", record_id, e)
            raise DataAccessError(
                message="获取记录失败", details={"record_id": record_id, "error": str(e)}
            ) from e
    ...
    def get_by_user_mode_version(
        ...
        try:
            ...
            logger.error(
                "获取记录失败: user_id=%s, mode=%s, version=%s, error=%s", user_id, mode, version, e
            )
            raise DataAccessError(
                message="获取记录失败",
    ...
    def get_all_by_user_mode(self, user_id: int, mode: str) -> list[dict[str, Any]]:
        ...
        try:
            df = self.db.query(
                self.TABLE_NAME, where={"user_id": user_id, "mode": mode}, order_by="version DESC"
            )
            if df.empty:
                return []
    ...
    def create(self, data: dict[str, Any]) -> int | None:
        """
        创建新记录
        ...
        try:
            ...
            logger.error("创建记录失败: error=%s", e)
            raise DataAccessError(message="创建记录失败", details={"error": str(e)}) from e
    ...
    def create_new_version(
        ...
        try:
            ...
            logger.error("创建新版本失败: user_id=%s, mode=%s, error=%s", user_id, mode, e)
            raise DataAccessError(
                message="创建新版本失败",
                details={"user_id": user_id, "mode": mode, "error": str(e)},
            ) from e
    ...
    def update(self, record_id: int, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("更新记录失败: record_id=%s, error=%s", record_id, e)
            raise DataAccessError(
                message="更新记录失败", details={"record_id": record_id, "error": str(e)}
            ) from e
    ...
    def update_by_user_mode_version(
        ...
        try:
            ...
            if rows_affected > 0:
                logger.info(
                    "更新 Being 记录成功: user_id=%s, mode=%s, version=%s", user_id, mode, version
                )
                return True
            return False
        except sqlite3.Error as e:
            logger.error(
                "更新记录失败: user_id=%s, mode=%s, version=%s, error=%s", user_id, mode, version, e
            )
            raise DataAccessError(
                message="更新记录失败",
                details={"user_id": user_id, "mode": mode, "version": version, "error": str(e)},
    ...
    def upsert(
        ...
        try:
            ...
            logger.info(
                "UPSERT Being 记录成功: user_id=%s, mode=%s, version=%s", user_id, mode, version
            )
            return True
        except sqlite3.Error as e:
            logger.error(
    ...
    def delete(self, record_id: int) -> bool:
        ...
        try:
            ...
            logger.error("删除记录失败: record_id=%s, error=%s", record_id, e)
            raise DataAccessError(
                message="删除记录失败", details={"record_id": record_id, "error": str(e)}
            ) from e
    ...
    def delete_by_user_mode_version(self, user_id: int, mode: str, version: int) -> bool:
        ...
        try:
            rows_affected = self.db.delete(
                self.TABLE_NAME, where={"user_id": user_id, "mode": mode, "version": version}
            )
            if rows_affected > 0:
                logger.info(
                    "删除 Being 记录成功: user_id=%s, mode=%s, version=%s", user_id, mode, version
                )
                return True
            return False
        except sqlite3.Error as e:
            logger.error(
                "删除记录失败: user_id=%s, mode=%s, version=%s, error=%s", user_id, mode, version, e
            )
            raise DataAccessError(
                message="删除记录失败",
                details={"user_id": user_id, "mode": mode, "version": version, "error": str(e)},
