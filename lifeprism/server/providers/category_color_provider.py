...
class CategoryColorManager:
    ...
    def initialize_colors(self) -> None:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            for cat_id, _cat_name, base_color, state in main_categories:
                if state == 0:
                    # 禁用的主分类使用浅灰色
                    ...
                    # 启用的主分类使用数据库中的颜色
                    self._main_category_colors[cat_id] = base_color or self._get_default_color(
                        cat_id
                    )
            ...
            for main_cat_id, subs in sub_by_main.items():
                ...
                for idx, (sub_id, _sub_name, _) in enumerate(subs):
    ...
    def _soften_color(self, hex_color: str) -> str:
        ...
        # 调整亮度和饱和度使颜色更柔和
        # 亮度提高到 0.70 ~ 0.78 范围
        new_l = 0.70 + (lightness * 0.1)  # 基础亮度 0.70，略微保留原亮度差异
        ...
        return f"#{int(max(0, min(1, r_new)) * 255):02x}{int(max(0, min(1, g_new)) * 255):02x}{int(max(0, min(1, b_new)) * 255):02x}"
    ...
    def _lightest_color(self, hex_color: str) -> str:
        ...
        new_l = 0.90 + (lightness * 0.05)  # 基础亮度 0.90
    ...
    def get_timeline_main_color(self, category_id: str) -> str:
        ...
        return self._timeline_main_colors.get(
            category_id, self._soften_color(self._get_default_color(category_id))
        )
    ...
    def get_custom_block_main_color(self, category_id: str) -> str:
        ...
        return self._custom_block_main_colors.get(
            category_id, self._lightest_color(self._get_default_color(category_id))
        )
    ...
    def _generate_color_variants(self, base_color: str, count: int, level: int = 2) -> list[str]:
        ...
        if count <= 1:
            # 单个颜色时，根据层级选择亮度
            single_l = 0.55 if level == 2 else 0.75
            r_new, g_new, b_new = colorsys.hls_to_rgb(h, single_l, s)
            return [
                f"#{int(max(0, min(1, r_new)) * 255):02x}{int(max(0, min(1, g_new)) * 255):02x}{int(max(0, min(1, b_new)) * 255):02x}"
            ]
        ...
        for i in range(count):
            new_l = max_l - (max_l - min_l) * (i / (count - 1)) if count > 1 else lightness
            ...
            hex_color = f"#{int(max(0, min(1, r_new)) * 255):02x}{int(max(0, min(1, g_new)) * 255):02x}{int(max(0, min(1, b_new)) * 255):02x}"
