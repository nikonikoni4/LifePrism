...
class ServerLWDataProvider(LWBaseDataProvider):
    ...
    def get_category_stats(self, date: str, category_type: str = "category") -> list[dict]:
        ...
        if category_type not in ("category", "sub_category"):
            raise ValueError(
                f"无效的 category_type: {category_type}，只支持 'category' 或 'sub_category'"
            )
        ...
        if category_type == "category":
            ...
            meta_dict = {
                str(row[0]): {"name": row[1], "category_id": str(row[2])} for row in meta_rows
            }
            return [
                {
                    "id": str(row[0]),
    ...
    def get_app_usage_summary(self, start_time: str = None, end_time: str = None) -> pd.DataFrame:
        """
        获取应用使用时长汇总
    ...
    def get_tokens_usage(
        self, date: str = None, start_time: str = None, end_time: str = None
    ) -> dict[str, dict]:
        """
        获取token使用汇总
    ...
    def get_tokens_usage_by_mode(
        self, date: str = None, start_time: str = None, end_time: str = None, mode: str = None
    ) -> dict[str, dict]:
        """
    ...
    def update_category_map_cache_by_id(self, record_id: str, update_fields: dict) -> bool:
        """
    ...
    def batch_update_category_map_cache_by_ids(
        self, record_ids: list[str], update_fields: dict
    ) -> int:
        """
    ...
    def delete_category_map_cache_by_id(self, record_id: str) -> bool:
        """
    ...
    def batch_delete_category_map_cache_by_ids(self, record_ids: list[str]) -> int:
        """
