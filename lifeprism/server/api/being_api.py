...
@router.get("/{mode}", response_model=BeingTestResponse, summary="获取最新版本测试")
def get_latest_test(
...
@router.get("/{mode}/versions", response_model=VersionListResponse, summary="获取所有版本列表")
def get_versions(
    mode: Literal["past", "present", "future"] = Path(..., description="模式"),
    user_id: int = Query(default=1, description="用户 ID"),
):
...
@router.get("/{mode}/{version}", response_model=BeingTestResponse, summary="获取指定版本测试")
def get_test_by_version(
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404, detail=f"未找到 {mode} 模式版本 {version} 的测试记录"
            )
        return result
    except LWBaseError:
        raise
...
@router.post("/{mode}", response_model=BeingTestResponse, summary="创建新版本测试")
def create_test(
    mode: Literal["past", "present", "future"] = Path(..., description="模式"),
    ...
    try:
        ...
        logger.error(
            "创建%s模式测试记录失败: user_id=%s, error=%s", mode, user_id, e, exc_info=True
        )
        raise HTTPException(status_code=500, detail="服务器内部错误") from e


@router.put("/{mode}/{version}", response_model=SuccessResponse, summary="更新指定版本测试")
def update_test(
    mode: Literal["past", "present", "future"] = Path(..., description="模式"),
    version: int = Path(..., description="版本号"),
    body: BeingTestContent = None,
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail=f"更新失败，未找到 {mode} 模式版本 {version} 的记录"
            )
        ...
        logger.error(
            "更新%s模式测试记录失败: user_id=%s, version=%s, error=%s",
            mode,
            user_id,
            version,
            e,
...
@router.delete("/{mode}/{version}", response_model=SuccessResponse, summary="删除指定版本测试")
def delete_test(
    mode: Literal["past", "present", "future"] = Path(..., description="模式"),
    version: int = Path(..., description="版本号"),
    user_id: int = Query(default=1, description="用户 ID"),
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail=f"删除失败，未找到 {mode} 模式版本 {version} 的记录"
            )
        ...
        logger.error(
            "删除%s模式测试记录失败: user_id=%s, version=%s, error=%s",
            mode,
            user_id,
            version,
            e,
...
@router.post(
    "/{mode}/{version}/ai-abstract", response_model=SuccessResponse, summary="生成 AI 总结"
)
