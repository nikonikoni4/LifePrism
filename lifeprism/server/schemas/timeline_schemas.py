...
class TimelineBlockStats(BaseModel):
    ...
    categories: list[TimelineCategoryStats] = Field(
        default=[], description="该时间块内的分类统计（按时长降序排列）"
    )
...
class TimelineStatsResponse(BaseModel):
    ...
    blocks: list[TimelineBlockStats] = Field(default=[], description="时间块列表（按小时顺序排列）")
...
class UserCustomBlockCreate(BaseModel):
    ...
    category_id: str | None = Field(None, description="主分类ID（可选）")
    sub_category_id: str | None = Field(None, description="子分类ID（可选）")
    todo_id: str | None = Field(None, description="关联的待办事项ID（可选）")
    color: str | None = Field(None, description="活动颜色（可选，前端随机生成 Tailwind 200 系列）")


class UserCustomBlockUpdate(BaseModel):
    """更新用户自定义数据块的请求体（所有字段可选）"""

    content: str | None = Field(None, description="活动内容描述")
    start_time: str | None = Field(None, description="开始时间（ISO格式：YYYY-MM-DDTHH:MM:SS）")
    end_time: str | None = Field(None, description="结束时间（ISO格式：YYYY-MM-DDTHH:MM:SS）")
    duration: int | None = Field(None, description="持续时长（分钟）")
    category_id: str | None = Field(None, description="主分类ID")
    sub_category_id: str | None = Field(None, description="子分类ID")
    todo_id: str | None = Field(None, description="关联的待办事项ID")
    color: str | None = Field(None, description="活动颜色")
...
class UserCustomBlock(BaseModel):
    ...
    content: str = Field(..., description="活动内容描述")
    todo_id: str | None = Field(None, description="关联的待办事项ID")
    todo_content: str | None = Field(None, description="关联的待办事项内容（由后端查询填充）")
    start_time: str = Field(..., description="开始时间（ISO格式：YYYY-MM-DDTHH:MM:SS）")
    end_time: str = Field(..., description="结束时间（ISO格式：YYYY-MM-DDTHH:MM:SS）")
    duration: int = Field(..., description="持续时长（分钟）")
    category_id: str | None = Field(None, description="主分类ID")
    sub_category_id: str | None = Field(None, description="子分类ID")
    category: str | None = Field(None, description="主分类名称")
    sub_category: str | None = Field(None, description="子分类名称")
    color: str | None = Field(None, description="活动颜色（Tailwind 200 系列）")
...
class BehaviorAnalysisResponse(BaseModel):
    ...
    behavior_list: list[BehaviorAnalysisItem] = Field(
        default_factory=list, description="行为分析列表"
    )
