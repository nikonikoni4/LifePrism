...
class SubCategoryTreeItem(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"id": "coding", "name": "编程", "color": "#5B8FF9", "state": 1}
        }
    )
...
class CategoryTreeItem(BaseModel):
    ...
    color: str = Field(..., description="分类颜色（十六进制格式）")
    state: int = Field(default=1, description="分类状态（1: 启用, 0: 禁用）")
    subcategories: list[SubCategoryTreeItem] | None = Field(
        default=None, description="子分类列表，depth=1时为None"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "color": "#5B8FF9",
                "state": 1,
                "subcategories": [{"id": "coding", "name": "编程", "color": "#7C9AE6", "state": 1}],
            }
        }
    )
...
class TitleDuration(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"title": "Youtube - 首页", "duration": 300}}
    )
...
class AppUseInfo(BaseModel):
    ...
    top_titles: list[TitleDuration] | None = Field(
        default=None, description="该应用下使用时长最长的标题列表"
    )
...
class SubCategoryDef(BaseModel):
    ...
    app_use_info: list[AppUseInfo] | None = Field(
        default=None, description="该子分类下的应用使用情况"
    )
                ...
                "app_use_info": [
                    {
                        "name": "Code.exe",
                        "duration": 5400,
                        "top_titles": [{"title": "main.py - VS Code", "duration": 3000}],
                    }
                ],
            }
        }
...
class CategoryDef(BaseModel):
    ...
    subcategories: list[SubCategoryDef] | None = Field(
        default=None, description="子分类列表，depth=1时为None"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "duration_percent": 50,
                "subcategories": [
                    {"id": "coding", "name": "编程", "duration": 7200, "duration_percent": 50}
                ],
            }
        }
    )
...
class CategoryMapCacheItem(BaseModel):
    ...
    sub_category: str | None = Field(
        default=None, description="子分类名称（通过 sub_category_id 映射）"
    )
    # 分类 ID（原始值）
...
class DeleteCategoryMapCacheRequest(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"id": "m-a1b2c3d4"}})
...
class BatchDeleteCategoryMapCacheRequest(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"ids": ["m-a1b2c3d4", "s-e5f6g7h8"]}})
