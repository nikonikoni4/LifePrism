...
class ChartSegment(BaseModel):
    ...
    title: str | None = Field(default="", description="app层的标题显示")
...
class TimeOverviewData(BaseModel):
    ...
    total_range_minutes: int | None = Field(default=None, description="时间范围总分钟数")
    pie_data: list[ChartSegment] = Field(default=[], description="饼图数据")
    bar_keys: list[BarConfig] | None = Field(default=None, description="柱状图配置")
    bar_data: list[dict[str, Any]] | None = Field(default=None, description="时间分布数据")
    details: dict[str, "TimeOverviewData"] | None = Field(default=None, description="钻取详情")
...
class DailyReportResponse(BaseModel):
    ...
    sunburst_data: TimeOverviewData | None = Field(default=None, description="旭日图数据")
    todo_data: TodoStatsData | None = Field(default=None, description="Todo 统计数据")
    goal_data: list[GoalProgressData] | None = Field(default=None, description="Goal 进度数据")
    daily_trend_data: list[dict[str, Any]] | None = Field(
        default=None, description="24小时趋势数据"
    )
    comparison_data: Optional["ComparisonData"] = Field(
        default=None, description="与前一天的环比对比数据"
    )
    ai_summary: str | None = Field(default=None, description="AI 总结内容")
...
class UpsertDailyReportRequest(BaseModel):
    ...
    state: str | None = Field(default=None, description="数据状态")
...
class WeeklyReportResponse(BaseModel):
    ...
    goal_data: list[GoalProgressData] | None = Field(default=None, description="Goal 进度数据")
    daily_trend_data: list[dict[str, Any]] | None = Field(
        default=None, description="每日趋势数据（7天）"
    )
    comparison_data: Optional["ComparisonData"] = Field(
        default=None, description="与上一周的环比对比数据"
    )
    ai_summary: str | None = Field(default=None, description="AI 总结内容")
    state: str = Field(default="0", description="数据状态 (0: 未完成, 1: 已完成)")
    data_version: int = Field(default=1, description="数据格式版本号")
...
class UpsertWeeklyReportRequest(BaseModel):
    ...
    todo_data: TodoStatsData | None = Field(default=None, description="Todo 统计数据")
    goal_data: list[GoalProgressData] | None = Field(default=None, description="Goal 进度数据")
    daily_trend_data: list[dict[str, Any]] | None = Field(default=None, description="每日趋势数据")
    state: str | None = Field(default=None, description="数据状态")
...
class HeatmapDataItem(BaseModel):
    ...
    category_breakdown: dict[str, int] | None = Field(
        default=None, description="分类时间分解（分类名 -> 分钟数）"
    )
...
class MonthlyReportResponse(BaseModel):
    ...
    goal_data: list[GoalProgressData] | None = Field(default=None, description="Goal 进度数据")
    daily_trend_data: list[dict[str, Any]] | None = Field(
        default=None, description="每日趋势数据（按天）"
    )
    heatmap_data: list[HeatmapDataItem] | None = Field(default=None, description="热力图数据")
    comparison_data: Optional["ComparisonData"] = Field(
        default=None, description="与上一月的环比对比数据"
    )
    ai_summary: str | None = Field(default=None, description="AI 总结内容")
    state: str = Field(default="0", description="数据状态 (0: 未完成, 1: 已完成)")
    data_version: int = Field(default=1, description="数据格式版本号")
...
class AISummaryRequest(BaseModel):
    ...
    date: str = Field(..., description="日期 YYYY-MM-DD")
    pattern: str | None = Field(default="complex", description="总结模式，如 complex, simple")
...
class WeeklyAISummaryRequest(BaseModel):
    ...
    pattern: str | None = Field(
        default="custom", description="总结模式，可选值: simple, complex, custom"
    )
...
class MonthlyAISummaryRequest(BaseModel):
    ...
    month_end_date: str = Field(..., description="月结束日期 YYYY-MM-DD（月末）")
    pattern: str | None = Field(
        default="complex", description="总结模式，可选值: simple, complex, custom"
    )
...
class CategoryComparisonItem(BaseModel):
    ...
    change_percentage: float | None = Field(default=None, description="变化百分比（新增时为 null）")
    children: list["CategoryComparisonItem"] | None = Field(
        default=None, description="子分类对比列表"
    )
...
class ComparisonData(BaseModel):
    ...
    category_comparison: list[CategoryComparisonItem] = Field(
        default=[], description="分类对比列表"
    )
