...
from lifeprism.server.schemas.category_schemas import CategoryDef, CategoryTreeItem
...
class DailyActivitiesData(BaseModel):
    ...
    active_time_percentage: int = Field(
        ..., description="活动时长占比（%）", serialization_alias="activeTimePercentage"
    )
...
class ActivitySummaryData(BaseModel):
    ...
    daily_activities: list[DailyActivitiesData] = Field(
        ..., description="每日活动数据", alias="dailyActivities"
    )
    category_tree: list[CategoryTreeItem] | None = Field(
        default=None, description="分类树", alias="categoryTree"
    )
...
class BarConfig(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"key": "work", "label": "Work", "color": "#5B8FF9"}}
    )
...
class TimeOverviewData(BaseModel):
    ...
    total_tracked_minutes: int = Field(
        ..., alias="totalTrackedMinutes", description="总追踪时长（分钟）"
    )
    total_range_minutes: int | None = Field(
        default=None,
        alias="totalRangeMinutes",
        description="时间范围总分钟数（用于计算百分比的分母）",
    )
    ...
    details: dict[str, "TimeOverviewData"] | None = Field(
        None, description="子分类详情（递归结构）"
    )
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                ...
                "barKeys": [{"key": "work", "label": "Work", "color": "#5B8FF9"}],
                "barData": [{"timeRange": "0-2", "work": 0, "entertainment": 30, "other": 90}],
                "details": {
                    "Work/Study": {
...
class ActivityStatsResponse(BaseModel):
    ...
    activity_summary: ActivitySummaryData | None = Field(
        default=None, description="活动摘要条形图数据"
    )
    time_overview: TimeOverviewData | None = Field(default=None, description="时间概览数据")
    top_title: list[TopTitleData] | None = Field(default=None, description="热门标题数据")
    top_app: list[TopAppData] | None = Field(default=None, description="热门应用数据")
    category_tree: list[CategoryDef] | None = Field(default=None, description="分类树")
    query: dict[str, Any] | None = Field(default=None, description="查询参数回显（调试用）")
...
class ActivityLogItem(BaseModel):
    ...
    category_id: str | None = Field(default=None, description="主分类ID")
    sub_category_id: str | None = Field(default=None, description="子分类ID")
    category: str | None = Field(default=None, description="主分类")
    sub_category: str | None = Field(default=None, description="子分类")
    app_description: str | None = Field(None, alias="appDescription", description="应用描述")
    title_analysis: str | None = Field(None, alias="titleDescription", description="标题描述")
