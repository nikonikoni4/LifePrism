...
class UpdateMoodTypeRequest(BaseModel):
    """更新心情类型（部分更新）"""
    name: Optional[str] = Field(default=None, description="心情名称")
    icon: Optional[str] = Field(default=None, description="Lucide 图标名")
    color: Optional[str] = Field(default=None, description="十六进制颜色值")
    score: Optional[int] = Field(default=None, ge=0, le=100, description="心情评分权重 0-100")
    is_dark: Optional[int] = Field(default=None, description="是否深色主题")
...
class MoodEntryItem(BaseModel):
    ...
    content: Optional[str] = Field(default=None, description="用户输入的文字内容")
...
class CreateMoodEntryRequest(BaseModel):
    ...
    mood_type_id: str = Field(..., description="关联心情类型 ID")
    content: Optional[str] = Field(default=None, description="用户输入的文字内容")
    factors: Optional[List[str]] = Field(default=None, description="影响因素列表")


class UpdateMoodEntryRequest(BaseModel):
    """更新心情记录（部分更新）"""
    mood_type_id: Optional[str] = Field(default=None, description="关联心情类型 ID")
    content: Optional[str] = Field(default=None, description="用户输入的文字内容")
    factors: Optional[List[str]] = Field(default=None, description="影响因素列表")
