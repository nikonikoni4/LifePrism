...
class AWBaseDataProvider:
    ...
    def __init__(self, db_manager=None):
        ...
        if db_manager is None:
            from lifeprism.repository import aw_db_manager
    ...
    def get_buckets(self, bucket_type: str | None = None) -> list[dict]:
        """
        获取存储桶列表
        ...
        with self.db.get_connection() as conn:
            ...
            if bucket_type:
                cursor.execute("SELECT * FROM bucketmodel WHERE type = ?", (bucket_type,))
            else:
    ...
    def _get_bucket_key_by_type(self, bucket_type: str) -> str | None:
    ...
    def _get_events(
        self, bucket_key: str, start_time: str, end_time: str, limit: int = 500000
    ) -> list[dict]:
        """获取指定存储桶的事件数据"""
        with self.db.get_connection() as conn:
            cursor = conn.cursor()
