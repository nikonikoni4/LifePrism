...
4. **Bug 先测试**：修 bug 先写复现测试，再修复直到通过。
5. **记录教训**：每次被纠正后，在 CLAUDE.md 新增规则防止再犯。
6. **测试目录**：所有测试放在 test/ 目录下。
...
5. **路径统一通过 settings_manager**：
   ...
   - 路径优先级：环境变量 `LIFEPRISM_DATA_PATH` → yaml 配置 → 打包默认 `%APPDATA%/LifePrism/lifeprismData` → 开发默认 `frontend/lifeprismData`
