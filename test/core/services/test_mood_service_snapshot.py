...
def test_create_mood_type_snapshot(snapshot: SnapshotAssertion):
    ...
    request = CreateMoodTypeRequest(
        name="测试心情", icon="😊", color="#FF5733", score=80, is_dark=0, sort_order=100
    )
...
def test_update_mood_type_snapshot(snapshot: SnapshotAssertion):
    ...
    create_request = CreateMoodTypeRequest(
        name="待更新", icon="😐", color="#000000", score=50, is_dark=0, sort_order=50
    )
    created = mood_service.create_mood_type(create_request)

    # 更新
    update_request = UpdateMoodTypeRequest(name="已更新", score=60)
...
def test_delete_mood_type_snapshot(snapshot: SnapshotAssertion):
    ...
    # 先创建一个心情类型
    create_request = CreateMoodTypeRequest(
        name="待删除", icon="😢", color="#0000FF", score=20, is_dark=0, sort_order=10
    )
    created = mood_service.create_mood_type(create_request)
...
def test_get_mood_entries_with_date_range_snapshot(snapshot: SnapshotAssertion):
    """测试按时间范围获取心情记录"""
    result = mood_service.get_mood_entries(
        start_time="2026-01-01T00:00:00+00:00", end_time="2026-12-31T23:59:59+00:00"
    )
    assert result == snapshot
...
def test_create_mood_entry_snapshot(snapshot: SnapshotAssertion):
    ...
    type_request = CreateMoodTypeRequest(
        name="测试1", icon="😊", color="#00FF00", score=75, is_dark=0, sort_order=80
    )
    mood_type = mood_service.create_mood_type(type_request)

    # 创建心情记录
    entry_request = CreateMoodEntryRequest(
        mood_type_id=mood_type.id, content="今天心情不错", factors=["天气好", "工作顺利"]
    )
...
def test_update_mood_entry_snapshot(snapshot: SnapshotAssertion):
    ...
    # 先创建心情类型和记录
    type_request = CreateMoodTypeRequest(
        name="测试2", icon="😐", color="#FFFF00", score=50, is_dark=0, sort_order=50
    )
    mood_type = mood_service.create_mood_type(type_request)

    entry_request = CreateMoodEntryRequest(
        mood_type_id=mood_type.id, content="待更新的心情", factors=["因素1"]
    )
    created = mood_service.create_mood_entry(entry_request)

    # 更新
    update_request = UpdateMoodEntryRequest(content="已更新的心情", factors=["因素1", "因素2"])
...
def test_delete_mood_entry_snapshot(snapshot: SnapshotAssertion):
    ...
    # 先创建心情类型和记录
    type_request = CreateMoodTypeRequest(
        name="测试3", icon="😢", color="#FF0000", score=30, is_dark=0, sort_order=30
    )
    mood_type = mood_service.create_mood_type(type_request)

    entry_request = CreateMoodEntryRequest(
        mood_type_id=mood_type.id, content="待删除的心情", factors=[]
    )
    created = mood_service.create_mood_entry(entry_request)
...
def test_create_mood_impact_snapshot(snapshot: SnapshotAssertion):
    ...
    request = CreateMoodImpactRequest(name=f"测试因素{uuid.uuid4().hex[:4]}", sort_order=100)
...
def test_delete_mood_impact_snapshot(snapshot: SnapshotAssertion):
    """测试删除影响因素"""
    # 先创建一个影响因素
    create_request = CreateMoodImpactRequest(name="待删除因素", sort_order=50)
