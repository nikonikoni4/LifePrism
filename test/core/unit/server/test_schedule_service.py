...
class TestScheduleServiceIntegration:
    ...
    async def test_multiple_interval_jobs(self, schedule_service):
        ...
        # 添加两个不同间隔的任务
        job_id_30s = schedule_service.add_interval_job(
            test_func_30s, seconds=30, job_id="test_30s_multi"
        )
        job_id_1_2min = schedule_service.add_interval_job(
            test_func_1_2min, minutes=1.2, job_id="test_1.2min_multi"
        )
...
class TestScheduleServiceCronState:
    ...
    def test_save_multiple_cron_states(self, schedule_service_with_temp_state):
        ...
        # 加载状态
        state = schedule_service_with_temp_state._load_cron_state()
        assert state == {"update_memory": "2026-05-22", "another_job": "2026-05-23"}, (
            "应该包含两个任务的状态"
        )
    ...
    def test_should_execute_cron_today_no_record(self, schedule_service_with_temp_state):
        """测试没有执行记录时应该执行"""
        should_execute = schedule_service_with_temp_state._should_execute_cron_today(
            "update_memory"
        )
    ...
    def test_should_execute_cron_today_same_date(self, schedule_service_with_temp_state):
        ...
        # 使用本地日期匹配代码行为（_should_execute_cron_today 使用本地时区）
    ...
    def test_load_cron_state_corrupted_file(
        self, schedule_service_with_temp_state, temp_state_file
    ):
        """测试加载损坏的状态文件"""
        # 创建损坏的 JSON 文件
    ...
    async def test_execute_cron_with_state(self, schedule_service_with_temp_state):
        ...
        # 验证状态被保存（_execute_cron_with_state 使用本地日期记录）
        state = schedule_service_with_temp_state._load_cron_state()
        today = get_local_today().isoformat()
        assert state.get("test_job") == today, "执行后应该保存今天的本地日期"
