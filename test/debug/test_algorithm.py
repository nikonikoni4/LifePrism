                ...
                if version_content.startswith('```md'):
                    ...
                    if first_newline != -1:
                        ...
                        while pos <= len(version_content) - 3:
                            iteration += 1
                            if iteration <= 20 or version_content[pos:pos+3] == '```':  # 只打印前20次或遇到 ```
                                if version_content[pos:pos+3] == '```':
                                    print(f'位置 {pos}: 找到 ```, in_nested_block={in_nested_block}')
                            ...
                            if version_content[pos:pos+3] == '```':
                                if not in_nested_block:
                                    ...
                                    if next_char_pos >= len(version_content) or version_content[next_char_pos] == '\n':
                                        print(f'  -> 这是结束标记！closing_pos={pos}')
                                        closing_pos = pos
                                        break
                                    else:
                                        print(f'  -> 这是内部代码块开始，next_char={repr(version_content[next_char_pos])}')
                        ...
                        if closing_pos != -1:
                            ...
                            print(f'包含 "更新recent_state.md规则": {"更新recent_state.md规则" in extracted}')
                break
        break
