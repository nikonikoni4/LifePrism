...
class MoodSummaryTest(LLMTestBase):
    """心情总结测试类"""
    ...
    def __init__(
        ...
        """
        super().__init__(
            prompt=Prompts.Schedule.MOOD_SUMMARY,
    ...
    def _load_mood_data(self) -> list[dict]:
        """加载心情数据 JSON 文件"""
        mood_file = self.input_path / "mood" / "mood.json"
        if not mood_file.exists():
            raise FileNotFoundError(f"心情数据文件不存在: {mood_file}")

        with open(mood_file, "r", encoding="utf-8") as f:
            return json.load(f)

    def _format_mood_records(self, mood_data: list[dict]) -> str:
        ...
        for record in mood_data:
            ...
            factors_str = "、".join(factors) if factors else "无"
    ...
    def _build_messages(self, mood_records: str) -> list[dict[str, str]]:
...
{mood_records}
    ...
    def data_input(self, input_files: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        if input_files:
            date_groups = {
                date: records for date, records in date_groups.items() if date in input_files
            }
        ...
        for date in sorted(date_groups.keys()):
            ...
            formatted_records = self._format_mood_records(records)
            data_list.append(
                {
                    "mood_records": formatted_records,
    ...
    async def _process_single(self, data: dict) -> tuple[dict, TestLog]:
        """处理单个数据项"""
        messages = self._build_messages(data["mood_records"])
...
if __name__ == "__main__":
    # v1 版本测试
    test = MoodSummaryTest(prompt_version="v1", temperature=0.7)
    test.main()
