...
class BehaviorRawSummary(LLMTestBase):
    """行为原始数据总结测试类"""
    ...
    def __init__(
        ...
        """
        super().__init__(
            prompt=Prompts.Schedule.ACTIVITY_SUMMARY,
    ...
    def _get_behavior_files(self, input_files: list[str] | None = None) -> list[Path]:
        """获取行为数据文件列表"""
        ...
        if input_files:
            return [behavior_dir / f for f in input_files if (behavior_dir / f).exists()]
        else:
            return sorted(behavior_dir.glob("*.md"))

    def _build_messages(self, behavior_content: str, date: str) -> list[dict[str, str]]:
...
{behavior_content}
    ...
    def data_input(self, input_files: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        Returns:
            包含日期、内容和文件名的数据列表
        """
        behavior_files = self._get_behavior_files(input_files)
        data_list = []
...
if __name__ == "__main__":
    ...
    # 输入 prompt 版本
    prompt_version = input("Prompt 版本 [默认: v1]: ").strip()
    if not prompt_version:
        prompt_version = "v1"

    # 输入 temperature
    temperature_input = input("Temperature (0.0-1.0) [默认: 0.7]: ").strip()
    if temperature_input:
        try:
            temperature = float(temperature_input)
            if not (0.0 <= temperature <= 1.0):
                print("⚠️  Temperature 超出范围，使用默认值 0.7")
                temperature = 0.7
        except ValueError:
            print("⚠️  Temperature 格式错误，使用默认值 0.7")
            temperature = 0.7
    else:
        temperature = 0.7
    ...
    input_files = None
    if choice == "2":
        print("\n请输入文件名（多个文件用逗号分隔，例如: 2026-05-15.md, 2026-05-16.md）")
        files_input = input("文件名: ").strip()
        if files_input:
