...
class ActivitySummaryTest(LLMTestBase):
    ...
    def _get_activity_files(self, input_files: list[str] | None = None) -> list[Path]:
        """获取活动数据文件列表"""
        activity_dir = self.input_path / "activity_summary"
        if not activity_dir.exists():
            raise FileNotFoundError(f"活动数据目录不存在: {activity_dir}")

        if input_files:
            return [activity_dir / f for f in input_files if (activity_dir / f).exists()]
        else:
            return sorted(activity_dir.glob("*.md"))

    def _build_messages(self, activity_content: str, date: str) -> list[dict[str, str]]:
...
{activity_content}
    ...
    def data_input(self, input_files: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        Returns:
            包含日期、内容和文件名的数据列表
        """
        activity_files = self._get_activity_files(input_files)
        data_list = []
