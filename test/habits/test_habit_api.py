...
def patch_services(monkeypatch):
    """每个测试自动 patch 服务对象，避免污染集成测试。"""
    monkeypatch.setattr(habit_api_module, "habit_service", mock_habit_service)
    monkeypatch.setattr(habit_api_module, "habit_stats_service", mock_stats_module)
    monkeypatch.setattr(habit_api_module, "habit_chain_service", mock_habit_chain_service)
    yield
...
def test_checkin_today():
    ...
    resp = client.post("/habits/h-001/checkins")
    assert resp.status_code == 200
...
def test_checkin_habit_not_found():
    ...
    resp = client.post("/habits/h-notexist/checkins")
    assert resp.status_code == 404
    assert resp.json()["detail"]["error_code"] == "HABIT_NOT_FOUND"
...
def test_cancel_checkin_success():
    ...
    resp = client.delete("/habits/h-001/checkins/2026-03-01")
    assert resp.status_code == 200
...
def test_cancel_checkin_past_date():
    ...
    resp = client.delete("/habits/h-001/checkins/2026-02-01")
    assert resp.status_code == 422
...
def test_backfill_checkin_success():
    ...
    resp = client.post("/habits/h-001/checkins/backfill", json={
...
def test_get_backfill_availability_success():
    hs.get_backfill_availability.return_value = MagicMock()
    hs.get_backfill_availability.side_effect = None
    resp = client.post("/checkins/backfill/availability", json={
        "habitId": "habit-001",
        "challengeId": "challenge-001",
    })
    assert resp.status_code == 200
...
def test_get_backfill_availability_challenge_not_found():
    hs.get_backfill_availability.side_effect = NotFoundError("挑战不存在")
    resp = client.post("/checkins/backfill/availability", json={
        "habitId": "habit-001",
        "challengeId": "challenge-not-found",
    })
    assert resp.status_code == 404
    assert resp.json()["detail"]["error_code"] == "CHALLENGE_NOT_FOUND"
...
def test_check_settlements():
    ...
    resp = client.post("/check-settlements")
    assert resp.status_code == 200
...
def test_create_node_validation_failed():
    hcs.create_node.side_effect = ValidationError("èŠ‚ç‚¹æ—¶é—´é¡ºåºä¸åˆç†")
    resp = client.post("/chains/1/nodes", json={"name": "èŠ‚ç‚¹", "triggerTime": "07:00"})
    assert resp.status_code == 422
...
def test_update_node_validation_failed():
    hcs.update_node.side_effect = ValidationError("èŠ‚ç‚¹æ—¶é—´é¡ºåºä¸åˆç†")
    resp = client.patch("/chains/1/nodes/1", json={"triggerTime": "06:00"})
    assert resp.status_code == 422
